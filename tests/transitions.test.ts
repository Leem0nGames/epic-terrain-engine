...
`;
    ...
    // Water hex at (0,0) should have a grass transition from direction 2 (SE) where grass is
    // Grass is at (1,0) which is direction 2 (SE) from (0,0) in the new order (N, NE, SE, S, SW, NW)
    const waterCell = grid.get(0, 0);
    expect(waterCell).toBeDefined();
    
    // Grass has higher z-index, so water cell should have transition mask bit 2 set for Gg
    expect(waterCell!.transitionMasks.get('Gg')! & (1 << 2)).toBeGreaterThan(0);
